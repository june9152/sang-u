...
public class SignalHandler extends TextWebSocketHandler {
    ...
    //웹소켓 연결이 종료되었을 때 호출되는 메소드로 연결이 종료된 세션과 상태를 로그로 기록
    ...
    }//웹소켓 연결이 성립되었을 때 호출되는 메소드//연결이 성공하면 클라이언트에게 MSG_TYPE_JOIN 메시지를 전송하며 이 메시지는 현재 방이 비어있지 않음을 알리는 메시지
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage textMessage) {
        ...
        try {
            ...
            switch (message.getType()) {
                //switch 문은 특정 변수의 값을 검사하고 그 값에 따라 다른 작업을 수행하는 구조임
                //message.getType()의 값이 같으면 case구문이 실행
                //클라이언트에서 받은 message.getType()에따라 다른 작업이 수행
                ...
                case MSG_TYPE_ICE:
                    ...
                    //각각의 메시지에서 SDP 정보와 ICE 후보 정보를 추출하고, 해당 정보를 로깅함
                            ...
                            if (!client.getKey().equals(userUUID)) {
                                ...
                                sendMessage(client.getValue(),
                                        new WebSocketMessage(
                                                ...
                                                candidate,
                                                sdp));
                                //현재 메시지를 보낸 클라이언트를 제외한 모든 클라이언트에게 메시지를 전달
                            }
                        }
                    }
                    break;
//
                ...
                // MSG_TYPE_JOIN의 경우,새로운 사용자가 채팅방에 입장하였을 때 호출
...
//                            .orElseThrow(() -> new IOException("Invalid room number received!"));
                ...
                case MSG_TYPE_LEAVE:
                    ...
                    Optional<String> client = rtcChatService.getClients(room).keySet().stream()
                            ...
                            // 3. 하여튼 동일한 것만 가져온다
                        //room의 모든 클라이언트 목록에서 현재 사용자를 찾음
                            .findAny();
    ...
    //WebSocketMessage 객체를 JSON 문자열로 변환한 후, WebSocket 세션을 통해 이를 전송
}
